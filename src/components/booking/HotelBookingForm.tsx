...
export default function HotelBookingForm({ hotel, onClose }: HotelBookingFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const pricePerNight = parseInt(hotel.price.replace(/[₦,]/g, ''), 10);
      const checkInDate = new Date(formData.checkIn);
      const checkOutDate = new Date(formData.checkOut);
      const nights = Math.ceil((checkOutDate.getTime() - checkInDate.getTime()) / (1000 * 60 * 60 * 24));
      const totalPrice = pricePerNight * nights * parseInt(formData.rooms, 10);
      ...
      if (orderResult.success) {
        ...
        sessionStorage.setItem('hotelOrderId', orderResult.order.id);
        sessionStorage.setItem('hotelOrderData', JSON.stringify(orderResult.order));

        // Redirect to checkout
        router.push(`/checkout?type=hotel&name=${encodeURIComponent(hotel.name)}&price=${totalPrice}&location=${encodeURIComponent(hotel.location)}&orderId=${orderResult.order.id}`);
      } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      ...
      >
        <div className="sticky top-0 bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between z-10">
          <h2 className="text-2xl font-bold text-[#1A1830]">Book {hotel.name}</h2>
          <button
            onClick={onClose}
